...
const Team = () => {
  ...
  return (
    <section id="team" className="py-24 relative overflow-hidden bg-brand-darker grid-bg border-b border-black/[0.04]">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 ${
              activeTab === 'faculty'
                ...
                : 'bg-white text-slate-500 hover:text-slate-800 border border-black/[0.04]'
            }`}
            data-hover="true"
          ...
          <AnimatePresence mode="wait">
            {teamData[activeTab].map((person, index) => (
              <motion.div
                ...
                className="bg-white border border-black/[0.04] p-6 rounded-3xl flex flex-col justify-between h-full relative group overflow-hidden shadow-[0_15px_40px_rgba(15,23,42,0.02)] hover:shadow-lg transition-all duration-300"
              >
                ...
                <div>
                  ...
                  <div className="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:text-brand-blue group-hover:border-brand-blue/30 transition-all duration-300 font-sans font-bold text-lg mb-6 shadow-inner">
                    {person.initials}
                  </div>

                  <h3 className="text-lg font-bold text-slate-800 mb-1 font-sans">{person.name}</h3>
                  <p className="text-[10px] font-bold text-brand-red uppercase tracking-wide mb-4">
                    {person.role}
                  </p>

                  <div className="flex flex-col gap-2 border-t border-black/[0.03] pt-4">
                    ...
                    <div className="flex items-start gap-2.5 text-xs text-slate-500 italic mt-1 border-l-2 border-slate-200 pl-2">
                ...
                {/* Social Actions */}
                <div className="flex items-center gap-3 mt-8 pt-4 border-t border-black/[0.03]">
                  <a
                    href={person.linkedin}
                    className="p-2 rounded-lg bg-slate-50 hover:bg-slate-100 border border-black/[0.03] hover:border-brand-blue text-slate-400 hover:text-slate-700 transition-all duration-200"
                  ...
                  <a
                    href={person.email}
                    className="p-2 rounded-lg bg-slate-50 hover:bg-slate-100 border border-black/[0.03] hover:border-brand-red text-slate-400 hover:text-slate-700 transition-all duration-200"
